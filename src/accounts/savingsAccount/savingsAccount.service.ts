...
export class SavingsAccountService {
    ...
    create(data: Partial<SavingsAccount>): SavingsAccount {
        const newAccount = new SavingsAccount(data.accountNumber, data.balance, data.user);
    ...
    remove(accountNumber: string): void {
        const accountIndex = this.savingsAccounts.findIndex(acc => acc.accountNumber === accountNumber);
        if (accountIndex === -1) {
            throw new BadRequestException('Could not find the account');
        }
